...
def main():
    ...
    dev_path = PROJECT_ROOT / cfg["data_root"]
    ...
    ####################################################################################################################################
    dev_path = r"D:\Facultate\DL4NLP\Project\data\dev_phase\subtask1\dev"

    for file in os.listdir(dev_path):
            print(file)
            dev_filee = os.path.join(dev_path, file)
            dev_df = pd.read_csv(dev_filee)
            ...
            out = pd.DataFrame({cfg["id_col"]: dev_df[cfg["id_col"]].values, "polarization": dev_pred})
            pred_file = "pred_" + file
            out.to_csv(pred_file, index=False)
            print("Saved dev predictions to:", pred_file)
