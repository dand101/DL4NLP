...
def main():
    ...
    # --------------------
    wandb.init(
        project=cfg.get("wandb_project", "polarization"),
    ...
    cfg["outputs_dir"] = os.path.join(cfg["outputs_dir"], run_id)
    ...
    # LOAD DATA
    # --------------------
    train_path = str(PROJECT_ROOT / cfg["data_root"] / cfg["train_file"])
    dev_path = str(PROJECT_ROOT / cfg["data_root"] / cfg["dev_file"])
    ...
    # --------------------
    # SPLIT (USES SWEEP VALUES)
    # --------------------
    cfg["balance_test"] = str(cfg["balance_test"]).lower() == "true"
    cfg["use_class_weights"] = str(cfg["use_class_weights"]).lower() == "true"
