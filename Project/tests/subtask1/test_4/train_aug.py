...
def main():
    ...
    to_add = n_majority - n_minority

    print(f"Majority label: {majority_label} ({n_majority})")
    print(f"Minority label: {minority_label} ({n_minority})")
    print(f"Need to add {to_add} samples of label {minority_label}")

    aug_df = pd.read_csv("train_augmented_backtranslation.csv")

    aug_minority = aug_df[aug_df[label_col] == minority_label]
